...
export type FlowAccessor<F, T> = (flow: F) => T;
...
export interface FlowAccessors<F> {
    ...
    getFlowTime?: FlowAccessor<F, Date>;
}
...
export type FlowmapDataAccessors<L, F> = LocationAccessors<L> & FlowAccessors<F>;
...
export declare function isCluster(c: ClusterNode): c is Cluster;
export declare function isLocationClusterNode<L>(l: L | ClusterNode): l is ClusterNode;
...
export declare function isAggregateFlow(flow: Record<string, any>): flow is AggregateFlow;
export interface FlowCountsMapReduce<F, T = any> {
    map: (flow: F) => T;
    reduce: (accumulated: T, val: T) => T;
}
export declare enum LocationFilterMode {
    ALL = "ALL",
    INCOMING = "INCOMING",
    OUTGOING = "OUTGOING",
    BETWEEN = "BETWEEN"
...
export type LayersDataAttrValues<T> = {
    value: T;
    size: number;
};
